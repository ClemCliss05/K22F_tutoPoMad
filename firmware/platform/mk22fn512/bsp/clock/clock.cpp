...
// MCG->S->IREFST=0 // permits to be sure Source of FLL reference clock is the external reference
// clock MCG->S->CLKST=10 // permits to be sure External reference clock is selected
// // FBE State: FLL Bypassed External (8MHz)
// // setXXXXHz()
// // DIVIDE THE CLOCK BEFORE TO ENABLE PLL (DON'T FORGET FREQUENCY LIMITATIONS)
// SIM->CLKDIV1->OUTDIV1=0000 // 0000 => divide by 1, core clock = 120MHz (Up to 120 MHz)
// SIM->CLKDIV1->OUTDIV2=0001 // 0001 => divide by 2, bus clock = 60MHz (Up to 60 MHz)
// SIM->CLKDIV1->OUTDIV3=0001 // 0001 => divide by 4, flexbus clock = 30MHz (Up to 30 MHz)
// SIM->CLKDIV1->OUTDIV4=0100 // 0100 => divide by 5, flash clock = 24MHz (Up to 26.67 MHz)
// MCG->C5->PRDIV0=00010 // 00010 => 3, 8/3= 2.6666666667MHz
// MCG->C6->VDIV0=10101 // 10101 => 45, 45 * 2.6666666667MHz = 120MHz, see SystemCoreClockUpdate
// (void) MCG->C6->PLLS=1 // activate PLL -> transition FBE to PBE MCG->S->PLLST=1 // permits to be
// sure Source of PLLS clock is PLL output clock. MCG->S->LOCK0=1 // permits to be sure PLL is
// locked
...
bool Bsp::Clock::initOSC() {
    uint32_t timeout = 1000000;

    // FEI -> FBE
    MCG->C2 &= ~(MCG_C2_RANGE_MASK | MCG_C2_EREFS0_MASK | MCG_C2_HGO_MASK);
    MCG->C2 |= MCG_C2_RANGE(0b01) | MCG_C2_EREFS0_MASK | MCG_C2_HGO(0b0);
    MCG->C1 &= ~(MCG_C1_CLKS_MASK | MCG_C1_IREFS_MASK);
    MCG->C1 |= MCG_C1_CLKS(0b10) | MCG_C1_IREFS(0b0);
    ...
    do {
        ...
        if ((MCG->S & MCG_S_OSCINIT0_MASK) == MCG_S_OSCINIT0(0b1) &&
            !(MCG->S & MCG_S_IREFST_MASK) && (MCG->S & MCG_S_CLKST_MASK) == MCG_S_CLKST(0b10)) {
...
// Set PLL to 48MHz
bool Bsp::Clock::set48MHz() {
    uint32_t timeout = 1000000;
    bool PBE_status = false;

    // DIVIDE THE CLOCK BEFORE TO ENABLE PLL (DON'T FORGET FREQUENCY LIMITATIONS)
    SIM->CLKDIV1 &= ~(SIM_CLKDIV1_OUTDIV1_MASK | SIM_CLKDIV1_OUTDIV2_MASK |
                      SIM_CLKDIV1_OUTDIV3_MASK | SIM_CLKDIV1_OUTDIV4_MASK);
    SIM->CLKDIV1 |= SIM_CLKDIV1_OUTDIV1(0b0000) | SIM_CLKDIV1_OUTDIV2(0b0000) |
                    SIM_CLKDIV1_OUTDIV3(0b0001) | SIM_CLKDIV1_OUTDIV4(0b0001);

    // FBE -> PBE
    MCG->C5 &= ~MCG_C5_PRDIV0_MASK;
    MCG->C5 |= MCG_C5_PRDIV0(0b00011); // 8MHz / 4 = 2MHz
    MCG->C6 &= ~(MCG_C6_VDIV0_MASK | MCG_C6_PLLS_MASK);
    MCG->C6 |= MCG_C6_VDIV0(0b00000) | // (24) * 2MHz = 48MHz
    ...
    do {
        ...
        if ((MCG->S & MCG_S_PLLST_MASK) && (MCG->S & MCG_S_LOCK0_MASK)) {
            PBE_status = true;
            break;
        }
...
bool Bsp::Clock::set120MHz() {
    ...
    SIM->CLKDIV1 &= ~(SIM_CLKDIV1_OUTDIV1_MASK | SIM_CLKDIV1_OUTDIV2_MASK |
                      SIM_CLKDIV1_OUTDIV3_MASK | SIM_CLKDIV1_OUTDIV4_MASK);
    SIM->CLKDIV1 |= SIM_CLKDIV1_OUTDIV1(0b0000) | SIM_CLKDIV1_OUTDIV2(0b0001) |
                    SIM_CLKDIV1_OUTDIV3(0b0011) | SIM_CLKDIV1_OUTDIV4(0b0100);

    // FBE -> PBE
    MCG->C5 &= ~MCG_C5_PRDIV0_MASK;
    MCG->C5 |= MCG_C5_PRDIV0(0b00010); // 8MHz / 3 = 2.6666666667MHz
    MCG->C6 &= ~(MCG_C6_VDIV0_MASK | MCG_C6_PLLS_MASK);
    MCG->C6 |= MCG_C6_VDIV0(0b10101) | // (45) * 2.6666666667MHz = 120MHz
               MCG_C6_PLLS_MASK;       // activate PLL
